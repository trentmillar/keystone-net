...
namespace Keystone.Server.Internal
{
    ...
    public sealed partial class KeystoneServerProvider : OpenIdConnectServerProvider
    {
        private async Task CreateAuthorizationAsync([NotNull] AuthenticationTicket ticket,
            [NotNull] KeystoneServerOptions options, [NotNull] OpenIdConnectRequest request)
        {
            var descriptor = new KeystoneAuthorizationDescriptor
            {
                Principal = ticket.Principal,
                Status = KeystoneConstants.Statuses.Valid,
                Subject = ticket.Principal.GetClaim(KeystoneConstants.Claims.Subject),
                Type = KeystoneConstants.AuthorizationTypes.AdHoc
            };
        ...
        private async Task<bool> TryRevokeAuthorizationAsync([NotNull] AuthenticationTicket ticket)
        {
            ...
            catch (KeystoneExceptions.ConcurrencyException exception)
            {
        ...
        private async Task<bool> TryExtendRefreshTokenAsync(
            [NotNull] object token, [NotNull] AuthenticationTicket ticket, [NotNull] KeystoneServerOptions options)
        {
            ...
            OpenIdConnectRequest request, AuthenticationProperties properties)
        {
            ...
            foreach (var property in properties.Items)
            {
                ...
                if (property.Key.EndsWith(KeystoneConstants.PropertyTypes.Boolean))
                {
                    var name = property.Key.Substring(
                        startIndex: 0,
                        length: property.Key.LastIndexOf(KeystoneConstants.PropertyTypes.Boolean));
                ...
                else if (property.Key.EndsWith(KeystoneConstants.PropertyTypes.Integer))
                {
                    var name = property.Key.Substring(
                        startIndex: 0,
                        length: property.Key.LastIndexOf(KeystoneConstants.PropertyTypes.Integer));
                ...
                else if (property.Key.EndsWith(KeystoneConstants.PropertyTypes.Json))
                {
                    var name = property.Key.Substring(
                        startIndex: 0,
                        length: property.Key.LastIndexOf(KeystoneConstants.PropertyTypes.Json));
                ...
                else if (property.Key.EndsWith(KeystoneConstants.PropertyTypes.String))
                {
                    var name = property.Key.Substring(
                        startIndex: 0,
                        length: property.Key.LastIndexOf(KeystoneConstants.PropertyTypes.String));
